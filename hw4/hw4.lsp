...
;
;Helper Function: iterates through creates a list of lists, each of which is (-i -j) where i = c1 for node n and j = every color from c2 to k for node n
(defun negative-pairs (n c1 c2 k)
  (cond
   ((> c2 k) nil)
   (t (cons
       (list (* -1 (node2var n c1 k)) (* -1 (node2var n c2 k)))
       (negative-pairs n c1 (+ c2 1) k)
       ))
   )
  )
...
(defun at-most-one-color (n c k)
  (cond
   ((> c k) nil)
   (t (append (negative-pairs n c (+ c 1) k) (at-most-one-color n (+ c 1) k)))
   )
  )
...
;
;Helper Function: creates edge clauses recursively, from colors c to k
(defun edge-clause-helper (e c k)
  (cond
   ((> c k) nil)
   (t (cons
       (list (* -1 (node2var (car e) c k)) (* -1 (node2var (cadr e) c k)))
       (edge-clause-helper e (+ c 1) k)
       ))
   )
  )
